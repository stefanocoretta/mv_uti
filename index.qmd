...
frame_la <- expand_grid(
  Subject = unique(la_f$Subject),
  vow_emph = unique(la_f$vow_emph),
...
la_gam_p |> 
  ggplot(aes(Knot, X, colour = Consonant.Emphaticness)) +
  geom_point() +
  facet_grid(rows = vars(Vowel)) +
  theme(legend.position = "bottom")
la_gam_p |> 
  ggplot(aes(Knot, Y, colour = Consonant.Emphaticness)) +
  geom_point() +
  facet_grid(rows = vars(Vowel)) +
  theme(legend.position = "bottom")
```
...
la_gam_p |> 
  ...
  # ) |>
  ggplot(aes(X, Y, colour = Consonant.Emphaticness)) +
  geom_errorbarh(aes(xmin = X_lo, xmax = X_hi), alpha = 0.05, linewidth = 2.5) +
  geom_errorbar(aes(ymin = Y_lo, ymax = Y_hi), alpha = 0.05, linewidth = 1) +
  geom_point(size = 1, alpha = 0.75) +
  scale_color_brewer(type = "qual", palette = "Dark2") +
  coord_fixed() +
  facet_wrap(vars(Vowel), ncol = 3) +
  theme_light() +
  theme(legend.position = "bottom")
...
pred_grid_a <- filter(
  frame_la, vow_emph == "E.Plain",
  Subject == "Sak"
)
pred_grid_b <- filter(
  frame_la, vow_emph == "E.Emphatic",
  Subject == "Sak"
)

pred_a <- predict.gam(la_gam, pred_grid_a, type = "lpmatrix") |> 
  as_tibble() |> 
  mutate(
    across(starts_with("s(knot,Subject)"), ~0)
  ) |> 
  as.matrix()
# pred_a[,301:603] <- 0
pred_a[,1:300] <- 0

pred_b <- predict.gam(la_gam, pred_grid_b, type = "lpmatrix") |> 
  as_tibble() |> 
  mutate(
    across(starts_with("s(knot,Subject)"), ~0)
  ) |> 
...
pred_diff <- pred_a - pred_b
diff <- as.vector(pred_diff %*% stats::coef(la_gam))

se <- sqrt(rowSums((pred_diff %*% stats::vcov(la_gam)) * pred_diff))
