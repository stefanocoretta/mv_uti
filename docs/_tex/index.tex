...
produce the same type of error. While using polar coordinates would work
mathematically, it is less ideal from an articulatory perspective: a
polar-based fan-line system with a single origin fails to capture the
the vectors of movement of the individual knots estimated by DLC.
Figure~\ref{fig-vect} illustrates this point: the polar radii in grey
...
bi-dimensional space, the representation of displacement of each knot is
constrained by the direction of the radii. We don't consider this
constraint to be fatal, but Generalised Additive Models allow for a more
