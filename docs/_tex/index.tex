...
vectors (with the exception of the fifth knot, which can be regarded as
the pivot knot), represented in the figure by black segments. The
vectors of articulatory movement make sense in light of the muscular
anatomy of the tongue (\citeproc{ref-honda1996}{Honda, 1996};
...
the direction of the radii. We don't consider this constraint to be
fatal, but Multivariate Generalised Additive Models allow for a more
accurate depiction of knot movement by removing such constraints.
...
deal-breaking, property). In this section, we will illustrate GAMs
applied to the X/Y trajectories along the knots and how we can
reconstruct the tongue contour from the modelled trajectories. We will
use data from two case studies of coarticulation: vowel-consonant (VC)
coarticulation based on consonant place in Italian and Polish, and
consonantal articulation of plain vs emphatic consonants in Lebanese
Arabic.
...
the variable does not have to be \emph{time}-varying. The variation can
be on any linear variable: in the case of DLC-tracked UTI data, the
...
dimension \(N \times 11\), where \(N\) is the number of tongue contours
...
compute the first two PCs, but you can compute up to the number of DLC
knots in the data, i.e.~11.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Number of PC to compute}
\NormalTok{n\_pc }\OtherTok{\textless{}{-}} \DecValTok{2}
\CommentTok{\# You can compute up to 11 PCs.}
\CommentTok{\# n\_pc \textless{}{-} 11}
...
of each PC with the following code.
...
along a vertical/horizontal axis, as mentioned in
...
(green lines in the left panel) suggests lowering of the tongue
...
course also possible) with PC1 as the outcome, emphaticness (emphatic vs
plain) and vowel (A, E, I, O, U) and their interaction as predictors. We
also included by-participant varying intercepts and varying slopes for
the emphaticness/vowel interaction. Both emphaticness and vowel were
coded using sum contrasts. This is the R formula we used:
...
Table~\ref{tbl-dlc-voff} shows the mean, standard deviation and
...
\bottomrule\noalign{}
\endlastfoot
A\_diff & -0.58 & 0.25 & -1.03 & -0.08 & -0.83 & -0.33 & -0.77 &
-0.39 \\
E\_diff & -0.18 & 0.25 & -0.68 & 0.34 & -0.44 & 0.09 & -0.38 & 0.03 \\
I\_diff & -0.22 & 0.42 & -1.06 & 0.59 & -0.65 & 0.24 & -0.55 & 0.14 \\
O\_diff & -0.38 & 0.23 & -0.81 & 0.09 & -0.62 & -0.14 & -0.57 & -0.20 \\
U\_diff & -1.08 & 0.49 & -2.06 & -0.07 & -1.60 & -0.52 & -1.49 &
-0.64 \\
\end{longtable}
...
distribution (despite being a safe neutral assumption).
...
similar interpretations. This has not been systematically tested,
...
\url{https://github.com/uasolo/FPCA-phonetics-workshop/tree/master/presentations/UTI}.
